...
foreach ($gender in $(if ($CrowdOnly) { @() } else { @('Female', 'Male') })) {
...
& $blender --background --factory-startup --python-exit-code 1 --python (Join-Path $PSScriptRoot 'prepare-crowd-assets.py')
if ($LASTEXITCODE -ne 0) { throw 'Crowd preparation failed.' }
